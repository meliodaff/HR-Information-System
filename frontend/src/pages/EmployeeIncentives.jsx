          ...
          {activeTab === 'incentives' && (
            <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-xl shadow-lg p-6">
              ...
              <div className="bg-white rounded-xl p-6 mb-6 shadow-md">
                ...
                <p className="text-sm text-gray-600 mb-4">here are your rewards</p>
                
                {/* Rewards Summary */}
                <div className="bg-blue-50 rounded-lg p-4 mb-6">
                  <p className="text-sm text-gray-700">
                    You're <span className="font-bold">50% towards</span> perfect attendance this month
                  </p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center">
                    ...
                    <h4 className="font-bold text-sm text-gray-900">Employee</h4>
                    <p className="text-xs text-gray-600">of the month</p>
                    <p className="text-xs text-gray-500 mt-1">{achievementsCount.employeeOfMonth}</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{achievementsCount.perfectAttendance}</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <h4 className="font-bold text-sm text-gray-900">Outstanding</h4>
                    <p className="text-xs text-gray-600">Care Award</p>
                    <p className="text-xs text-gray-500 mt-1">{achievementsCount.outstandingCare}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-black text-gray-900 uppercase">Date</th>
                        <th className="px-4 py-3 text-left text-xs font-black text-gray-900 uppercase">Reward</th>
                        <th className="px-4 py-3 text-left text-xs font-black text-gray-900 uppercase">Remarks/Bonus</th>
                      ...
                      {filteredRewards.length > 0 ? (
                        filteredRewards.map((reward, index) => (
                          <tr key={index} className="hover:bg-gray-50 transition-colors">
                            <td className="px-4 py-3 text-sm text-gray-900">{reward.date}</td>
                            <td className="px-4 py-3 text-sm text-gray-900">{reward.reward}</td>
                            <td className="px-4 py-3 text-sm text-gray-900">{reward.remarksBonus}</td>
                            <td className="px-4 py-3">
                              <span className={`px-4 py-1 rounded-full text-xs font-semibold ${getStatusColor(reward.status)}`}>
                                {reward.status}
                              </span>
                            </td>
                          </tr>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredBenefits.length > 0 ? (
                  filteredBenefits.map((benefit, index) => (
                    <div key={index} className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-xl shadow-md p-6">
                      <h3 className="text-2xl font-black text-gray-900 mb-4">{benefit.name}</h3>
                      
                      <div className="space-y-2 mb-4">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-700">Monthly:</span>
                          <span className="font-bold text-gray-900">{benefit.monthly}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-700">Total Contribution:</span>
                          <span className="font-bold text-gray-900">{benefit.totalContribution}</span>
                        </div>
                      </div>
